...
import streams.jmx.client.jobConfig.JobConfigOverlay;
...
public class SubmitJob extends AbstractInstanceCommand {
    ...
    protected CommandResult doExecute() {
        try {
            ...
            String jobConfigOverlayString = null;
            if (this.configSettings.size() > 0) {
                // MAJOR Change for V5
                // The submitjob call no longer takes in configuration (-C) parameters directly
                // Instead, we need to either
                // 1) If a config overlay is provided, override with supported -C parameters
            ...
            //Put Job Configuration Overlay if provided or created
            if (jobConfigOverlayString != null) {
                // We generated or updated the job config and it is a String
                LOGGER.debug("Sending job config overlay string to HTTP Server");
                getJmxServiceContext().getWebClient().putString(deployInformation.getUris().get(1),
                    "application/json", jobConfigOverlayString, getConfig().getJmxHttpHost(), getConfig().getJmxHttpPort());
            } else if (jobConfigFile != null) {
                ...
                LOGGER.debug("No job config overlay provided or generated by config settings");
            }
