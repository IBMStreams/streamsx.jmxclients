...
public class MetricsExporter {
	...
	static private String getOperatorOutputPortMetricFullName(String metricName) {
		return "streams_operator_op_" + metricName;
	}
	...
	static public void createOperatorOutputPortMetric(String metricName, String description) {
		String metricFullName = getOperatorOutputPortMetricFullName(metricName);
		if (!gaugeMap.containsKey(metricFullName)) {
			gaugeMap.put(metricFullName, Gauge.build()
							.name(metricFullName)
							.help(description)
							.labelNames("instancename","jobname","operatorname","outputportname")
							.register());
		}
	}	
	...
	static public Metric getOperatorOutputPortMetric(String metricName, String streamsInstanceName, String jobName, String operatorname, String outputportname) {
		String metricFullName = getOperatorOutputPortMetricFullName(metricName);
		if (!gaugeMap.containsKey(metricFullName)) {
			// Create with default help text
			createOperatorOutputPortMetric(metricName, "Streams operator output port metric: " + metricName);
		}
		return new Metric(metricFullName, Arrays.asList(streamsInstanceName, jobName, operatorname, outputportname));		
	}	
	...
	static public void removeOperatorOutputPortMetric(String metricName, String streamsInstanceName, String jobName, String operatorname, String outputportname) {
		String metricFullName = getOperatorOutputPortMetricFullName(metricName);
		if (gaugeMap.containsKey(metricFullName)) {
			Gauge g = gaugeMap.get(metricFullName);
			g.remove(new String[]{streamsInstanceName,jobName,operatorname,outputportname});
